module Core.Permutation

(*
  Permutations of [0..n) — proofs and internal helpers.

  Public interface lives in `Core.Permutation.fsti`.
...
let rec fwd_agree_implies_perm_eq_bool_from
  ...
  (pf: (i: fin n) -> Lemma (p.fwd i == q.fwd i))
  : Lemma (ensures  perm_eq_bool_from p q k)
  = if k >= n then ()
    else (pf (k <: fin n); fwd_agree_implies_perm_eq_bool_from p q (k + 1) pf)

let perm_eq_intro (#n: nat) (p q: permutation n)
                  (pf: (i: fin n) -> Lemma (p.fwd i == q.fwd i))
  : Lemma (ensures perm_eq p q)
  = reveal_opaque (`%perm_eq) (perm_eq p q);
    fwd_agree_implies_perm_eq_bool_from p q 0 pf
...
let identity_left (#n: nat) (p: permutation n)
  : Lemma (perm_eq (compose (identity n) p) p) = perm_eq_intro (compose (identity n) p) p (fun _ -> ())

let identity_right (#n: nat) (p: permutation n)
  : Lemma (perm_eq (compose p (identity n)) p) = perm_eq_intro (compose p (identity n)) p (fun _ -> ())

let compose_associative (#n: nat) (p q r: permutation n)
  : Lemma (perm_eq (compose (compose p q) r) (compose p (compose q r))) = perm_eq_intro (compose (compose p q) r) (compose p (compose q r)) (fun _ -> ())
...
let inverse_left (#n: nat) (p: permutation n)
  ...
  perm_eq_intro lhs (identity n) goal
...
let inverse_involutive (#n: nat) (p: permutation n)
  : Lemma (perm_eq (inverse (inverse p)) p) = perm_eq_intro (inverse (inverse p)) p (fun _ -> ())
...
let inverse_congruence (#n: nat) (p q: permutation n)
  ...
  = let goal (j: fin n) : Lemma ((inverse p).fwd j == (inverse q).fwd j)
    ...
    perm_eq_intro (inverse p) (inverse q) goal
...
let transposition_self_inverse (n: nat) (a b: fin n)
  : Lemma (perm_eq (compose (transposition n a b) (transposition n a b))
                   (identity n)) = perm_eq_intro (compose (transposition n a b) (transposition n a b)) (identity n) (fun _ -> ())

let transposition_trivial (n: nat) (a: fin n)
  : Lemma (perm_eq (transposition n a a) (identity n)) = perm_eq_intro (transposition n a a) (identity n) (fun _ -> ())
...
(* ------------------------------------------------------------------------ *)
(*  Typeclass instances: ring + mul_group (permutation n)             *)
(*  Atomic raw fields per new-design Bundles shape.                         *)
(* ------------------------------------------------------------------------ *)
...
let compose_congruence (#n: nat) (p1 q1 p2 q2: permutation n)
  ...
  = let aux (i: fin n) : Lemma ((compose p1 q1).fwd i == (compose p2 q2).fwd i) =
    ...
    perm_eq_intro (compose p1 q1) (compose p2 q2) aux

(* The old tower exposed `permutation n` as a `mul_monoid`/`mul_group`
   typeclass instance. In the new tower (Core.Algebra) the only
   multiplicative-bundle classes are `ring` / `commutative_ring` etc.,
   which require ADDITIVE structure — and there is no natural addition
   on permutations. So we drop the typeclass instances and expose
   `compose`, `identity`, `inverse`, `perm_eq` as plain top-level
   functions/predicates, called by name in downstream code. *)
...
let right_swap_fwd_at_i (#n: nat) (p: permutation n) (i: nat{i+1 < n})
  : Lemma ((right_swap p i).fwd (i <: fin n) == p.fwd ((i+1) <: fin n))
  = ()

let right_swap_fwd_at_i_plus_1 (#n: nat) (p: permutation n) (i: nat{i+1 < n})
  : Lemma ((right_swap p i).fwd ((i+1) <: fin n) == p.fwd (i <: fin n))
  = ()

let right_swap_fwd_at_other (#n: nat) (p: permutation n) (i: nat{i+1 < n}) (k: fin n)
  : Lemma (requires (k <: nat) <> i /\ (k <: nat) <> i+1)
          (ensures (right_swap p i).fwd k == p.fwd k)
  = ()
...
let parity_compose_right_swap
  ...
  = Classical.forall_intro (compose_with_right_swap_eq p q i);
    perm_eq_intro (compose p (right_swap q i)) (right_swap (compose p q) i) (compose_with_right_swap_eq p q i);
...
let rec sign_homomorphism
  ...
  = match find_descent q 0 with
    | None ->
        ...
        let aux_cpq (k: fin n) : Lemma ((compose p q).fwd k == p.fwd k) = aux_q k in
        perm_eq_intro (compose p q) p aux_cpq;
...
(*  Its parity is (m * n) mod 2 (i., m*n inversions).                    *)
(* ====================================================================== *)
...
let block_swap_perm_fwd_in_first_block (m n: nat) (k: fin (m + n))
  : Lemma (requires (k <: nat) < m)
          (ensures (block_swap_perm m n).fwd k == (k <: nat) + n)
  = ()

let block_swap_perm_fwd_in_second_block (m n: nat) (k: fin (m + n))
  : Lemma (requires (k <: nat) >= m)
          (ensures (block_swap_perm m n).fwd k == (k <: nat) - m)
  = ()
