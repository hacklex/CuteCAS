...
let a_times_bterm (#t:Type) {| cr: commutative_ring t |} (a b: t) (n:nat{n>=1}) (k:nat{k <= n-1})
  : Lemma (a * bterm a b (n-1) k
           = nat_scale (NB.binom (n-1) k) (rpow a (n-k) * rpow b k))
  = H.elim_equatable_laws t (); H.trans_for_calc t ();
    let c = NB.binom (n-1) k in
    let i = n-1-k in
    assert (bterm a b (n-1) k == nat_scale c (rpow a i * rpow b k));
    nat_scale_mul_right c a (rpow a i * rpow b k);
    pull_a a b i k;
    assert ((i ++ 1) == n-k);
    nat_scale_congruence c
        (a * (rpow a i * rpow b k)) (rpow a (n-k) * rpow b k);
    H.trans2 (a * bterm a b (n-1) k)
             (nat_scale c (a * (rpow a i * rpow b k)))
             (nat_scale c (rpow a (n-k) * rpow b k))
...
let b_times_bterm (#t:Type) {| cr: commutative_ring t |} (a b: t) (n:nat{n>=1}) (k:nat{1 <= k /\ k <= n})
  : Lemma (b * bterm a b (n-1) (k-1)
           = nat_scale (NB.binom (n-1) (k-1)) (rpow a (n-k) * rpow b k))
  = H.elim_equatable_laws t (); H.trans_for_calc t ();
    let c = NB.binom (n-1) (k-1) in
    let i = n-k in
    assert ((n-1)-(k-1) == i);
    assert (bterm a b (n-1) (k-1) == nat_scale c (rpow a i * rpow b (k-1)));
    nat_scale_mul_right c b (rpow a i * rpow b (k-1));
    pull_b a b i (k-1);
    assert (((k-1) ++ 1) == k);
    nat_scale_congruence c
        (b * (rpow a i * rpow b (k-1))) (rpow a i * rpow b k);
    H.trans2 (b * bterm a b (n-1) (k-1))
             (nat_scale c (b * (rpow a i * rpow b (k-1))))
             (nat_scale c (rpow a i * rpow b k))
...
let bterm_pascal (#t:Type) {| cr: commutative_ring t |} (a b: t) (n:nat{n>=2}) (k:nat{1 <= k /\ k <= n-1})
  ...
  = H.elim_equatable_laws t (); H.trans_for_calc t ();
    ...
    a_times_bterm a b n k;     (* a*bterm(n-1,k)   = nat_scale c1 x *)
    b_times_bterm a b n k;     (* b*bterm(n-1,k-1) = nat_scale c2 x *)
    (* sum of the two nat_scale terms *)
    add_congruence (a * bterm a b (n-1) k) (b * bterm a b (n-1) (k-1))
                   (nat_scale c1 x) (nat_scale c2 x);
    (* nat_scale c1 x + nat_scale c2 x = nat_scale (c1+c2) x *)
    nat_scale_add c1 c2 x;    (* nat_scale (c1+c2) x = ns c1 x + ns c2 x *)
    symmetry (nat_scale ((c1 ++ c2)) x)
             (nat_scale c1 x + nat_scale c2 x);
    (* c1 + c2 = C(n,k) by Pascal *)
    NB.pascal (n-1) k;                (* C(n-1,k) + C(n-1,k-1) = C(n,k) *)
    assert ((c1 ++ c2) == NB.binom n k);
    (* bterm a b n k = nat_scale C(n,k) x  (since k<=n, the if picks n-k) *)
    assert (bterm a b n k == nat_scale (NB.binom n k) x);
...
let bterm_corner_0 (#t:Type) {| cr: commutative_ring t |} (a b: t) (n:nat)
  ...
  = H.elim_equatable_laws t (); H.trans_for_calc t ();
    ...
    assert (bterm a b n 0 == nat_scale 1 (rpow a n * rpow b 0));
    nat_scale_one (rpow a n * rpow b 0);   (* nat_scale 1 X = X *)
    assert (rpow b 0 == one);
    H.x_mul_one (rpow a n);                        (* a^n * one = a^n *)

    mul_congruence (rpow a n) (rpow b 0) (rpow a n) one;  (* a^n * b^0 = a^n * one *)
...
let bterm_corner_n (#t:Type) {| cr: commutative_ring t |} (a b: t) (n:nat)
  ...
  = H.elim_equatable_laws t (); H.trans_for_calc t ();
    ...
    assert (bterm a b n n == nat_scale 1 (rpow a 0 * rpow b n));
    nat_scale_one (rpow a 0 * rpow b n);
    assert (rpow a 0 == one);
    H.one_mul_x (rpow b n);                        (* one * b^n = b^n *)

    mul_congruence (rpow a 0) (rpow b n) one (rpow b n);
...
let bterm_left_edge (#t:Type) {| cr: commutative_ring t |} (a b: t) (n:nat{n>=1})
  ...
  = H.elim_equatable_laws t (); H.trans_for_calc t ();
    bterm_corner_0 a b n;        (* bterm n 0 = a^n *)
    bterm_corner_0 a b (n-1);    (* bterm (n-1) 0 = a^(n-1) *)
...
let bterm_right_edge (#t:Type) {| cr: commutative_ring t |} (a b: t) (n:nat{n>=1})
  ...
  = H.elim_equatable_laws t (); H.trans_for_calc t ();
    bterm_corner_n a b n;          (* bterm n n = b^n *)
    bterm_corner_n a b (n-1);      (* bterm (n-1)(n-1) = b^(n-1) *)
...
let combined_is_bterm (#t:Type) {| cr: commutative_ring t |} (a b: t) (n:nat{n>=1}) (k:nat{k <= n})
  ...
  = H.elim_equatable_laws t (); H.trans_for_calc t ();
    if k = 0 then begin
      ...
      bterm_left_edge a b n;                      (* bterm n 0 = a*bterm(n-1,0) = afn0 *)
...
let sum_afn (#t:Type) {| cr: commutative_ring t |} (a b: t) (n:nat{n>=1})
  ...
  = H.elim_equatable_laws t (); H.trans_for_calc t ();
    ...
    sum_range_mul_left a (bterm a b (n-1)) 0 n;  (* a * sum = sum body *)

    H.trans2 (sum_range (afn a b n) 0 n)
             (sum_range body 0 n)
             (a * sum_range (bterm a b (n-1)) 0 n);
    H.trans2 (sum_range (afn a b n) 0 ((n ++ 1)))
             (sum_range (afn a b n) 0 n)
             (a * sum_range (bterm a b (n-1)) 0 n)

(* sum of the b-contribution over 0..n+1 equals  b · S(n-1). *)
let sum_bfn (#t:Type) {| cr: commutative_ring t |} (a b: t) (n:nat{n>=1})
  : Lemma (sum_range (bfn a b n) 0 ((n ++ 1))
           = b * sum_range (bterm a b (n-1)) 0 n)
  = H.elim_equatable_laws t (); H.trans_for_calc t ();
    (* peel the first (zero) term: sum 0 (n+1) = bfn 0 + sum 1 (n+1), bfn 0 = zero *)
    sum_range_unfold_left (bfn a b n) 0 ((n ++ 1));  (* = bfn 0 + sum 1 (n+1) *)
    assert (bfn a b n 0 == zero);

    add_congruence (bfn a b n 0) (sum_range (bfn a b n) 1 ((n ++ 1)))
                   zero (sum_range (bfn a b n) 1 ((n ++ 1)));
    H.zero_plus_x (sum_range (bfn a b n) 1 ((n ++ 1)));
    H.trans3 (sum_range (bfn a b n) 0 ((n ++ 1)))
             (bfn a b n 0 + sum_range (bfn a b n) 1 ((n ++ 1)))
             (zero + sum_range (bfn a b n) 1 ((n ++ 1)))
             (sum_range (bfn a b n) 1 ((n ++ 1)));
    (* shift:  sum_range (fun j -> bfn (j+1)) 0 n = sum_range bfn 1 (n+1) *)
    sum_range_shift (bfn a b n) 1 0 n;
    symmetry (sum_range (fun (j:nat) -> bfn a b n ((j ++ 1))) 0 n)
             (sum_range (bfn a b n) 1 ((n ++ 1)));
    (* on 0..n, bfn (j+1) = b*bterm(n-1,j) = body j *)
    let body : nat -> t = pointwise_mul (const b) (bterm a b (n-1)) in
    let h (j:nat{0 <= j /\ j < n}) : Lemma (bfn a b n ((j ++ 1)) = body j)
      = pointwise_mul_unfold (const b) (bterm a b (n-1)) j;
        const_unfold b j;
        assert (bfn a b n ((j ++ 1)) == b * bterm a b (n-1) j);
        reflexivity (b * bterm a b (n-1) j)
    in
    sum_range_congruence (fun (j:nat) -> bfn a b n ((j ++ 1))) body 0 n h;
    sum_range_mul_left b (bterm a b (n-1)) 0 n;   (* b * sum = sum body *)

    H.trans4 (sum_range (bfn a b n) 0 ((n ++ 1)))
             (sum_range (bfn a b n) 1 ((n ++ 1)))
             (sum_range (fun (j:nat) -> bfn a b n ((j ++ 1))) 0 n)
             (sum_range body 0 n)
             (b * sum_range (bterm a b (n-1)) 0 n)

(* the merge:  S(n) = a · S(n-1) + b · S(n-1). *)
let merge (#t:Type) {| cr: commutative_ring t |} (a b: t) (n:nat{n>=1})
  : Lemma (sum_range (bterm a b n) 0 ((n ++ 1))
           = a * sum_range (bterm a b (n-1)) 0 n
             + b * sum_range (bterm a b (n-1)) 0 n)
  = H.elim_equatable_laws t (); H.trans_for_calc t ();
    (* step1: bterm n = pointwise_add afn bfn  on 0..n+1 *)
    let h (k:nat{0 <= k /\ k < (n ++ 1)}) : Lemma (bterm a b n k = pointwise_add (afn a b n) (bfn a b n) k)
      = pointwise_add_unfold (afn a b n) (bfn a b n) k;     (* pa k = afn k + bfn k *)
        combined_is_bterm a b n k;                   (* afn k + bfn k = bterm n k *)
        symmetry (afn a b n k + bfn a b n k) (bterm a b n k)
    in
    sum_range_congruence (bterm a b n) (pointwise_add (afn a b n) (bfn a b n)) 0 ((n ++ 1)) h;
    (* step2: split *)
    sum_range_add (afn a b n) (bfn a b n) 0 ((n ++ 1));
    (* step3: evaluate each *)
    sum_afn a b n;
    sum_bfn a b n;
    add_congruence (sum_range (afn a b n) 0 ((n ++ 1)))
                   (sum_range (bfn a b n) 0 ((n ++ 1)))
                   (a * sum_range (bterm a b (n-1)) 0 n)
                   (b * sum_range (bterm a b (n-1)) 0 n);
    H.trans3 (sum_range (bterm a b n) 0 ((n ++ 1)))
             (sum_range (pointwise_add (afn a b n) (bfn a b n)) 0 ((n ++ 1)))
             (sum_range (afn a b n) 0 ((n ++ 1))
              + sum_range (bfn a b n) 0 ((n ++ 1)))
             (a * sum_range (bterm a b (n-1)) 0 n
              + b * sum_range (bterm a b (n-1)) 0 n)
...
let rec binomial_theorem (#t:Type) {| cr: commutative_ring t |} (a b: t) (n: nat)
  : Lemma (ensures rpow (a + b) n
                   = sum_range (bterm a b n) 0 ((n ++ 1)))
          (decreases n)
  = H.elim_equatable_laws t (); H.trans_for_calc t ();
    if n = 0 then begin
      (* rpow (a+b) 0 = one ; S(0) = bterm 0 0 = a^0 = one *)
      sum_range_singleton (bterm a b 0) 0;     (* S(0) = bterm 0 0 *)
      bterm_corner_0 a b 0;                      (* bterm 0 0 = a^0 = one *)
      assert (rpow a 0 == one);

      H.trans2 (rpow (a+b) 0) one (rpow a 0);
      (* now relate one to S(0): S(0) = bterm00 = a^0 = one, so rpow(a+b)0 = one = S(0) *)

      H.trans3 (rpow (a+b) 0) one (rpow a 0) (bterm a b 0 0);

      H.trans2 (rpow (a+b) 0) (bterm a b 0 0) (sum_range (bterm a b 0) 0 1)
    end
    else begin
      (* IH at n-1 *)
      binomial_theorem a b (n-1);                (* rpow(a+b)(n-1) = S(n-1) *)
      ...
      merge a b n;
      symmetry (sum_range (bterm a b n) 0 ((n ++ 1)))
