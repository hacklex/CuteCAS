...
let rec noncompact_poly_add #c (#r: commutative_ring c) (p q: noncompact_poly_over_ring r) 
  ...
  if is_empty p && is_empty q then empty
    ...
    assert (forall (i: nat{i>0 && i<max (length p) (length q)}). 
              nth sum i `r.eq` r.addition.op (nth (tail p) (i-1)) (nth (tail q) (i-1)));
    (r.addition.op (head p) (head q)) +$ (noncompact_poly_add (tail p) (tail q))
  

let nth_of_sum #c (#r: commutative_ring c) (p q: noncompact_poly_over_ring r) (n: nat) 
  : Lemma (nth (noncompact_poly_add p q) n `r.eq` r.addition.op (nth p n) (nth q n)) = ()  

let rec noncompact_poly_add_is_commutative #c (#r: commutative_ring c) (p q: noncompact_poly_over_ring r) 
  : Lemma (ensures (noncompact_poly_add p q) `noncompact_poly_eq` (noncompact_poly_add q p)) 
          (decreases length p + length q) = 
  reveal_opaque (`%is_reflexive) (is_reflexive #c); 
...
let rec noncompact_poly_add_is_associative #c (#r: commutative_ring c) (p q w: noncompact_poly_over_ring r) 
  ...
  if (is_nonempty p && is_nonempty q && is_nonempty w) then assoc_lemma_3 r.addition.op (head p) (head q) (head w);
  if (is_empty p && is_empty q && is_empty w) then () else 
  noncompact_poly_add_is_associative (tail p) (tail q) (tail w) 
  (* A verbose proof is no longer needed. Left intact just in case :)
    if (is_empty p && is_empty q && is_empty w) then () else 
...
let rec poly_nil_is_zero #c (#r: commutative_ring c) (p: noncompact_poly_over_ring r)
  : Lemma (ensures noncompact_poly_add p empty `noncompact_poly_eq` p) 
          (decreases length p) = 
  ...
  if is_nonempty p then poly_nil_is_zero (tail p)
...
let rec poly_add_respects_poly_eq #c (#r: commutative_ring c) 
  ...
  : Lemma (ensures noncompact_poly_eq (noncompact_poly_add x y) (noncompact_poly_add x z) /\
          ...
          (decreases length x + length y + length z) =
  let (| (+),           eq,   zero               |) = 
...
let poly_add_is_associative #c (#r: commutative_ring c) (p q w: poly_over_ring r) : Lemma (poly_add p (poly_add q w) `ncpoly_eq` poly_add (poly_add p q) w) = 
  ...
  calc (noncompact_poly_eq) {
    ...
    noncompact_poly_eq {} noncompact_poly_add p (poly_compact (noncompact_poly_add q w)); 
    noncompact_poly_eq {} noncompact_poly_add p (noncompact_poly_add q w); 
    noncompact_poly_eq {} noncompact_poly_add (noncompact_poly_add p q) w; 
    noncompact_poly_eq {} noncompact_poly_add (poly_compact (noncompact_poly_add p q)) w; 
    noncompact_poly_eq {} poly_compact (noncompact_poly_add (poly_compact (noncompact_poly_add p q)) w);    
  } 

(* This is a test that reasoning about rings will carry over to fields *)
...
(* This is unneeded as of March 9, 2022 *)
...
(* What follows is needed to establish connection to FStar.Algebra.CommMonoid.Equiv *)
...
let compact_poly_add_respects_poly_eq #c (#r: commutative_ring c) (x y z: poly_over_ring r)
  ...
  Classical.forall_intro_3 (ncpoly_eq_is_transitive_lemma #c #r);
  poly_add_respects_poly_eq x y z // F* handles all required ncpoly_eq transitivity facts 

(* We need this to construct an FStar.Algebra.CommMonoid.Equiv.cm *)
