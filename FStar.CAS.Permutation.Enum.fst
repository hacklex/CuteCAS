...
(*
   ...
   and pairwise-distinctness lemmas, then introduces a list-indexed sum
   `sum_over_perms` used by the determinant module.
*)

open FStar.CAS.Permutation

(* Local aliases for perm_eq sym/trans — the Permutation module now
   exposes these via the equatable instance rather than standalone vals. *)
...
(* xpand p n inserts p at every position 0..n, producing n+1 permutations
    of fin (n+1).  Used to lift one permutation of fin n into n+1 of fin (n+1). *)
...
(*                                                                      *)
(*  Given p : permutation (m+1), define a permutation educe p        *)
...
let bwd_distinct_from_top (#m: nat) (p: permutation (m+1)) (j': fin (m+1))
  ...
  = p.fwd_bwd_id j'   (* p.fwd (p.bwd j') == j'; if p.bwd j' = m then j' = p.fwd m *)
...
let insert_reduce_perm_eq (#m: pos) (p: permutation (m+1))
  ...
  = Classical.forall_intro (insert_reduce_pointwise p);
    perm_eq_intro (insert (reduce p) (p.fwd m)) p
...
let rec mem_concatMap
  ...
  = match xs with
    ...
    | h :: tl ->
        ...
        (* memP y (h :: tl) means y == h \/ memP y tl *)
        let aux () : Lemma (requires L.memP y tl) (ensures L.memP x (L.concatMap f tl))
          = mem_concatMap f y tl x in
        Classical.move_requires aux ()

(* expand p has length n+1 and contains insert p k for every k : fin (n+1).
   We prove a stronger fact: expand_aux p start contains insert p k
   for every k with start <= k <= n.  *)
...
let insert_congruence (#n: nat) (p q: permutation n) (k: fin (n+1))
  ...
  = Classical.forall_intro (Classical.move_requires (insert_congruence_pointwise p q k));
    perm_eq_intro (insert p k) (insert q k)

(* -------------------------------------------------------------------- *)
(*  Completeness of all_permutations.                                   *)
(* -------------------------------------------------------------------- *)

(* For n = 0, all_permutations 0 = [identity 0], and every permutation
   of fin 0 is trivially perm_eq to identity 0 (vacuous forall). *)
let completeness_base (p: permutation 0)
  : Lemma (permutation_in_list p (all_permutations 0))
  = perm_eq_intro p (identity 0);
...
let completeness_step (m: nat) (p: permutation (m+1))
  ...
  = match m with
    | 0 ->
        (* m = 0: reduce p : permutation 0; ih gives reduce p ~ identity 0,
           which is in all_permutations 0. Then insert (identity 0) (p.fwd 0)
           is perm_eq to p and lives in all_permutations 1.  Use base case
           directly without invoking pos-typed reduce. *)
        ...
        mem_concatMap (fun (r: permutation 0) -> expand r) q0 (all_permutations 0)
                      (insert q0 k);
        (* Show perm_eq p (insert q0 k): for i = 0, (insert q0 k).fwd 0 = k = p.fwd 0. *)
        perm_eq_intro p (insert q0 k)
    | _ ->
        let m_pos : pos = m in
        let k = p.fwd m in
        let p' : permutation m = reduce #m_pos p in
        ih p';
        (* Get a witness q' in all_permutations m with perm_eq p' q'. *)
...
(* -------------------------------------------------------------------- *)
(*  Pairwise-distinct predicate over perm_eq.                           *)
(* -------------------------------------------------------------------- *)
...
(* If the head of an expand_aux chain is perm_eq to any element of the
   tail, we get a contradiction (different k values give different fwd m). *)
...
(* Inverse of mem_concatMap: every element of concatMap f xs comes from
   some f y with y in xs. *)
...
(* The main no-dup theorem for all_permutations. *)
