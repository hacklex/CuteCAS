...
(*
   ...
   `FStar.CAS.Permutation.Enum`. This list has exactly `n!` elements,
...
let sum_over_perms_congruence
  ...
  = sum_list_map_congruence f g (all_permutations n)
...
let sum_over_perms_add_named
  ...
  = elim_equatable_laws t;
    ...
    let combined : permutation n -> t = fun p -> f p + g p in
    sum_over_perms_congruence n s combined;
    assert (sum_over_perms n s = sum_over_perms n combined);
    sum_over_perms_add n f g;
    assert (sum_over_perms n (fun p -> f p + g p) = sum_over_perms n f + sum_over_perms n g);
    assert (sum_over_perms n combined = sum_over_perms n f + sum_over_perms n g);
    transitivity (sum_over_perms n s)
                 (sum_over_perms n combined)
                 (sum_over_perms n f + sum_over_perms n g)
#pop-options
...
let sum_list_extract
  ...
  = let sp = sum_list pre in
    ...
    (* sum_list (pre ++ a :: post) = sp + (a + sq)   [defn of sum_list on cons] *)
...
let rec perm_eq_count_map_rmul
  ...
  : Lemma (ensures perm_eq_count p (L.map (fun s -> compose s q) xs) ==
...
let mapped_rmul_count_one (n: nat) (q: permutation n) (p: permutation n)
  : Lemma (perm_eq_count p (L.map (fun s -> compose s q) (all_permutations n)) == 1)
...
let rmul_q_yields_list_perm (n: nat) (q: permutation n)
  : Lemma (list_perm (all_permutations n)
                     (L.map (fun s -> compose s q) (all_permutations n)))
  = let aux (p: permutation n) : Lemma
        (perm_eq_count p (all_permutations n) ==
         perm_eq_count p (L.map (fun s -> compose s q) (all_permutations n)))
      = all_permutations_count_one n p;
...
let rec map_map_eq (#a #b #c: Type) (g: a -> b) (f: b -> c) (xs: list a)
  : Lemma (ensures L.map f (L.map g xs) == L.map (fun x -> f (g x)) xs)
          (decreases xs)
  = match xs with
    | [] -> ()
...
let sum_over_perms_reindex
  ...
  = rmul_q_yields_list_perm n q;
    sum_list_perm_invariance #t #m #n f
      (all_permutations n)
      (L.map (fun s -> compose s q) (all_permutations n));
    map_map_eq (fun s -> compose s q) f (all_permutations n)
#pop-options
...
private let rec sum_list_const_zero
  ...
  = match xs with
    ...
    | h :: tl ->
        ...
        reflexivity (zero #t);
        reflexivity (f h + sum_list (L.map f tl));
        reflexivity (sum_list (L.map f (h :: tl)));
        add_congruence (f h) (sum_list (L.map f tl)) (zero #t) (zero #t);
        left_add_identity (zero #t);
        trans_lemma [ sum_list (L.map f (h :: tl));
                      f h + sum_list (L.map f tl);
                      zero #t + zero #t;
                      zero #t ]
#pop-options
...
private let rec sum_list_pick
  (#n: nat) (#t: Type) {| m: add_comm_monoid t |}
...
private let rec sum_orbit_partition_zero
  ...
  = match xs with
    | [] ->
        sum_list_nil #t #(g.add_comm_monoid);
        reflexivity (zero #t)
    | h :: tl ->
        elim_equatable_laws t;
        transitivity_for_calc_proofs t;
