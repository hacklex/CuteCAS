...
let rec map_eval_map (#t:Type) {| cr: commutative_ring t |} (#n:nat)
  ...
  : Lemma (ensures L.map (fun (q: polynomial t) -> poly_eval q c) (L.map f l)
                   == L.map (fun (p: permutation n) -> poly_eval (f p) c) l)
          (decreases l)
  = match l with
    | [] -> ()
    | x :: rest -> map_eval_map f c rest
...
let eval_sum_over_perms (#t:Type) {| cr: commutative_ring t |} (n: nat)
  ...
  : Lemma (poly_eval
             (PS.sum_over_perms n f) c
           = PS.sum_over_perms n
               (fun (p: permutation n) -> poly_eval (f p) c))
  = H.elim_equatable_laws t ();
    PS.sum_over_perms_reveal n f;
    PS.sum_over_perms_reveal n
      (fun (p: permutation n) -> poly_eval (f p) c);
    ES.eval_sum_list (L.map f (all_permutations n)) c;
    map_eval_map f c (all_permutations n)
...
let eval_matrix (#t:Type) {| cr: commutative_ring t |} (#n:pos)
  ...
  = fun (i:fin n) (j:fin n) -> poly_eval (m i j) c
...
let perm_product_eval (#t:Type) {| cr: commutative_ring t |} (#n:pos)
  ...
  : Lemma (poly_eval
             (DET.perm_product m p) c
           = DET.perm_product (eval_matrix m c) p)
  = H.elim_equatable_laws t ();
    H.trans_for_calc t ();
    let ev : nat -> t = (fun (i:nat) -> poly_eval (DET.perm_entry m p i) c) in
    DET.perm_product_via m p;   (* perm_product m p == prod_range (perm_entry m p) 0 n *)
    DET.perm_product_via (eval_matrix m c) p;              (* perm_product (evalM) p == prod_range (perm_entry (evalM) p) 0 n *)
    ES.eval_prod_range (DET.perm_entry m p) c 0 n;         (* eval (prod_range (perm_entry m p)) = prod_range ev *)
    let h (i:nat{0 <= i /\ i < n}) : Lemma (ev i = DET.perm_entry (eval_matrix m c) p i)
      = H.elim_equatable_laws t ();
        assert (DET.perm_entry m p i == m i (p.fwd i));
        assert (ev i == poly_eval (m i (p.fwd i)) c);
        assert (DET.perm_entry (eval_matrix m c) p i == poly_eval (m i (p.fwd i)) c)
    in
    prod_range_congruence ev (DET.perm_entry (eval_matrix m c) p) 0 n h
#pop-options
...
let leibniz_eval (#t:Type) {| cr: commutative_ring t |} (#n:pos)
  (m: square_matrix (polynomial t) n) (p: permutation n) (c: t)
  : Lemma (poly_eval
             (DET.leibniz_term m p) c
           = DET.leibniz_term (eval_matrix m c) p)
  = H.elim_equatable_laws t ();
    perm_product_eval m p c;                                   (* eval(perm_product m p) c = perm_product (evalM) p *)
    if parity p then ()
    else begin
      let ppm = DET.perm_product m p in
      eval_neg ppm c;                                   (* eval(poly_neg ppm) c = neg (eval ppm c) *)
      neg_congruence
        (poly_eval ppm c)
        (DET.perm_product (eval_matrix m c) p);          (* neg (eval ppm c) = neg (perm_product (evalM) p) *)
      transitivity (poly_eval (DET.leibniz_term m p) c)
                   ((- (poly_eval ppm c)))
                   ((- (DET.perm_product (eval_matrix m c) p)))
    end
#pop-options
...
let det_eval (#t:Type) {| cr: commutative_ring t |} (#n:pos)
  ...
  : Lemma (poly_eval
             (DET.det m) c
           = DET.det (eval_matrix m c))
  = H.elim_equatable_laws t ();
    H.trans_for_calc t ();
    (* the poly add-group used by det's sum_over_perms is the canonical polynomial_acg *)
    assert (polynomial_acg cr == (polynomial_cr #t #cr).cr_r.r_add);
    let lt = DET.leibniz_term m in
    let lhsf : permutation n -> t =
      (fun (p: permutation n) -> poly_eval (lt p) c) in
    let lt' = DET.leibniz_term (eval_matrix m c) in
    DET.det_unfold m;   (* det m == sum_over_perms n lt *)
    DET.det_unfold (eval_matrix m c);                    (* det (evalM) == sum_over_perms n lt' *)
    eval_sum_over_perms n lt c;                          (* eval (sum_over_perms n lt) = sum_over_perms n lhsf *)
...
let resultant_eval (#t:Type) {| cr: commutative_ring t |}
  ...
  : Lemma (poly_eval
             (RES.resultant m_deg n_deg pp qq) c
           = DET.det
               (eval_matrix (SYL.sylvester_matrix m_deg n_deg pp qq) c))
  = RES.resultant_unfold m_deg n_deg pp qq;
    det_eval
      (SYL.sylvester_matrix m_deg n_deg pp qq) c
