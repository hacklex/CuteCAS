...
(* Pointwise agreement on fwd ⇒ prefix-equality (arg-form: caller supplies a
   pointwise lemma callback instead of stamping a forall in `requires`). *)
...
(*  Equatable, has_mul, has_one, has_inv are derived via Bundles' unfold    *)
(*  projections; only these two atomic-shaped instances are exposed.        *)
(* ------------------------------------------------------------------------ *)

(* Composition congruence over perm_eq (also used directly by downstream proofs). *)
...
(* Explicit rewrite of [.fwd] after a right_swap — three case lemmas. *)
val right_swap_fwd_at_i (#n: nat) (p: permutation n) (i: nat{i+1 < n})
  : Lemma ((right_swap p i).fwd (i <: fin n) == p.fwd ((i+1) <: fin n))

val right_swap_fwd_at_i_plus_1 (#n: nat) (p: permutation n) (i: nat{i+1 < n})
  : Lemma ((right_swap p i).fwd ((i+1) <: fin n) == p.fwd (i <: fin n))

val right_swap_fwd_at_other (#n: nat) (p: permutation n) (i: nat{i+1 < n}) (k: fin n)
...
val block_swap_perm_fwd_in_first_block (m n: nat) (k: fin (m + n))
  : Lemma (requires (k <: nat) < m)
          (ensures (block_swap_perm m n).fwd k == (k <: nat) + n)

val block_swap_perm_fwd_in_second_block (m n: nat) (k: fin (m + n))
  : Lemma (requires (k <: nat) >= m)
